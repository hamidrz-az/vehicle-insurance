...
const InsuranceCompaniesPage: React.FC = () => {
    ...
    const { data: insuranceCompanies, isLoading, error: insuranceCompaniesError } = useGetInsuranceCompaniesQuery(undefined);

    const { control, watch, formState: { errors, isValid } } = useForm<InsuranceCompanyValues>({
        ...
        <div>
            ...
            <form className="space-y-6">
                        ...
                        <InputField
                            ...
                            onChange={(e) => {
                                const selectedCompany = insuranceCompanies?.find(
                                    (company: SelectData) => company.id === parseInt(e.target.value)
                                );
                ...
                {errors.insuranceCompany && (
                    <p className="text-xs text-red-500 mt-1">{errors.insuranceCompany.message}</p>
                )}
