    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      >
        ...
        <div className="space-y-4">
          <InfoItem
            label="نوع بیمه"
            value={insuranceType === 'third-person' ? 'شخص ثالث' : 'بدنه'}
          />
          <InfoItem
            label="نوع خودرو"
            value={vehicleType?.title || 'انتخاب نشده است'}
          />
          <InfoItem
            label="مدل خودرو"
            value={vehicleModel?.title || 'انتخاب نشده است'}
          />
          <InfoItem
            label="شرکت بیمه گر قبلی"
            value={insuranceCompany?.title || 'انتخاب نشده است'}
          />
          <InfoItem
            label="تخفیف شخص ثالث"
            value={thirdPersonDiscount?.title || 'انتخاب نشده است'}
          />
          <InfoItem
            label="تخفیف حوادث راننده"
            value={driverDiscount?.title || 'انتخاب نشده است'}
          />
        </div>
        <Button
          variant="fill"
